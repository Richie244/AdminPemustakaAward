                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="p-6 border border-gray-200 rounded-xl bg-gray-50/50">
                        ...
                        <div id="skor-kunjungan-container" class="space-y-3 mb-4">
                            @php 
                                $kunjunganStarts = old('kunjungan_start', $previousSettings['kunjungan_start'] ?? []);
                            ...
                            @if(!empty($kunjunganStarts))
                                @foreach($kunjunganStarts as $index => $startValue)
                                    <div class="flex items-center gap-3 range-item">
                                        <input type="number" name="kunjungan_start[]" placeholder="Min Kunjungan" required class="block w-full rounded-lg border-gray-300 shadow-sm py-2 px-3 text-sm @error('kunjungan_start.'.$index) border-red-500 @enderror" value="{{ $startValue }}">
                                        <input type="number" name="kunjungan_end[]" placeholder="Maks Kunjungan" required class="block w-full rounded-lg border-gray-300 shadow-sm py-2 px-3 text-sm @error('kunjungan_end.'.$index) border-red-500 @enderror" value="{{ $kunjunganEnds[$index] ?? '' }}">
                                        <input type="number" name="kunjungan_skor[]" placeholder="Skor" required class="block w-full rounded-lg border-gray-300 shadow-sm py-2 px-3 text-sm @error('kunjungan_skor.'.$index) border-red-500 @enderror" value="{{ $kunjunganSkors[$index] ?? '' }}">
                                        <button type="button" onclick="hapusRange(this, 'btn-tambah-kunjungan')" class="p-1.5 text-red-500 hover:bg-red-100 rounded-full transition-colors flex-shrink-0">
                        ...
                        <button type="button" id="btn-tambah-kunjungan" onclick="tambahRange('skor-kunjungan-container', 'btn-tambah-kunjungan', 'kunjungan')" 
                         ...
                         @error('kunjungan_start.*') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                         @error('kunjungan_end.*') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                         @error('kunjungan_skor.*') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                    </div>
                    ...
                    <div class="p-6 border border-gray-200 rounded-xl bg-gray-50/50">
                            ...
                            @endphp
                             @if(!empty($pinjamanStarts))
                                @foreach($pinjamanStarts as $index => $startValue)
                                    <div class="flex items-center gap-3 range-item">
                                        <input type="number" name="pinjaman_start[]" placeholder="Min Pinjaman" required class="block w-full rounded-lg border-gray-300 shadow-sm py-2 px-3 text-sm @error('pinjaman_start.'.$index) border-red-500 @enderror" value="{{ $startValue }}">
                                        <input type="number" name="pinjaman_end[]" placeholder="Maks Pinjaman" required class="block w-full rounded-lg border-gray-300 shadow-sm py-2 px-3 text-sm @error('pinjaman_end.'.$index) border-red-500 @enderror" value="{{ $pinjamanEnds[$index] ?? '' }}">
                                        <input type="number" name="pinjaman_skor[]" placeholder="Skor" required class="block w-full rounded-lg border-gray-300 shadow-sm py-2 px-3 text-sm @error('pinjaman_skor.'.$index) border-red-500 @enderror" value="{{ $pinjamanSkors[$index] ?? '' }}">
                                        <button type="button" onclick="hapusRange(this, 'btn-tambah-pinjaman')" class="p-1.5 text-red-500 hover:bg-red-100 rounded-full transition-colors flex-shrink-0">
                                            <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path></svg>
                                        </button>
                                    </div>
                                @endforeach
                        ...
                        @error('pinjaman_start.*') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                        @error('pinjaman_end.*') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                        @error('pinjaman_skor.*') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                    </div>
                </div>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        @for ($i = 1; $i <= 3; $i++)
                            <div class="bg-white p-4 rounded-lg border border-gray-200 shadow-sm space-y-4">
                                ...
                                <div>
                                    <label for="reward_skor_{{ $i }}" class="block text-xs font-medium text-gray-500 mb-1">Skor Minimal Total untuk Level Ini</label>
                                    ...
                                    @error('rewards.'.$i.'.skor_minimal') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                                </div>
                                 <div>
                                    ...
                                    @error('rewards.'.$i.'.nama_reward') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                                </div>
                                 <div>
                                    ...
                                    @error('rewards.'.$i.'.slot_tersedia') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                                </div>
                            </div>
                        @endfor
                    </div>
                        ...
                        @if(isset($labelPoinKomponen) && is_array($labelPoinKomponen) && count($labelPoinKomponen) > 0)
                            @foreach($labelPoinKomponen as $key => $label)
                                <div>
                                    ...
                                    <input type="number" id="poin_komponen_{{ $key }}" name="poin_komponen[{{ $key }}]" placeholder="Poin/Bobot" required
                            ...
                            @endforeach
                        @else
                            <p class="text-sm text-gray-500 col-span-full">Data jenis bobot untuk poin komponen tidak tersedia.</p>
                        @endif
                    </div>
                </div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if ({{ old('kunjungan_start') ? count(old('kunjungan_start')) : 0 }} === 0 && (!previousSettings.kunjungan_start || previousSettings.kunjungan_start.length === 0)) {
            tambahRange('skor-kunjungan-container', 'btn-tambah-kunjungan', 'kunjungan');
        }
        if ({{ old('pinjaman_start') ? count(old('pinjaman_start')) : 0 }} === 0 && (!previousSettings.pinjaman_start || previousSettings.pinjaman_start.length === 0)) {
