    ...
    <div class="max-w-4xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex items-center gap-3 mb-6 pb-4 border-b border-gray-200">
            <a href="{{ route('kegiatan.index') }}"
                class="text-blue-600 hover:text-blue-800 inline-flex items-center p-2 rounded-full hover:bg-blue-50 transition-colors duration-150"
        ...
        <div class="bg-white p-6 shadow-xl rounded-xl overflow-hidden">
            ...
            @if ($currentTemplate && isset($currentTemplate->nama_file))
                ...
                <div class="mb-6 p-4 border rounded-lg bg-gray-50">
                    <p class="text-sm text-gray-600">Nama File:
                        <a href="{{ Storage::url('templates_sertifikat/' . $currentTemplate->nama_file) }}" target="_blank"
                    ...
                    @if (Str::endsWith(strtolower($currentTemplate->nama_file), ['.jpg', '.jpeg', '.png', '.gif']))
                        <div class="mt-4">
                            <img src="{{ Storage::url('templates_sertifikat/' . $currentTemplate->nama_file) }}"
                                alt="Preview Template" class="max-w-xs max-h-48 border rounded">
                        </div>
                    @endif
                ...
                <form action="{{ route('sertifikat-templates.store') }}" method="POST" enctype="multipart/form-data">
                    {{-- Atau route('sertifikat-templates.update', $currentTemplate->id_sertifikat) jika Anda punya metode update --}}
                    @csrf
                    {{-- Jika menggunakan metode UPDATE, tambahkan @method('PUT') --}}
                    {{-- @method('PUT') --}}

                    <div class="mb-4">
                        <label for="file_template_update" class="block text-sm font-medium text-gray-700 mb-1">Pilih File
                            Template Baru (JPG/PNG)</label>
                        <input type="file" name="file_template" id="file_template_update" required
                            accept=".jpg,.jpeg,.png"
                    ...
                    <div class="mb-4">
                        <label for="nama_template_update" class="block text-sm font-medium text-gray-700 mb-1">Nama Template
                            (Opsional, untuk referensi)</label>
            ...
            @else
                ...
                <p class="text-sm text-gray-600 mb-6">Belum ada template sertifikatyang diunggah. Silakan unggah template
                    pertama Anda.</p>

                <form action="{{ route('sertifikat-templates.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="mb-4">
                        <label for="nama_template_new" class="block text-sm font-medium text-gray-700 mb-1">Nama Template
                            (untuk referensi Anda)</label>
                        <input type="text" name="nama_template" id="nama_template_new"
                            value="{{ old('nama_template', 'Template Sertifikat') }}" required
                            class="border-gray-300 focus:border-blue-500 focus:ring-blue-500 rounded-md shadow-sm w-full py-2 px-3"
                    ...
                    <div class="mb-4">
                        <label for="file_template_new" class="block text-sm font-medium text-gray-700 mb-1">Pilih File
                            Template (JPG/PNG)</label>
                        <input type="file" name="file_template" id="file_template_new" required accept=".jpg,.jpeg,.png"
                            class="block w-full text-sm text-gray-500
                                  file:mr-4 file:py-2 file:px-4
                                  file:rounded-md file:border-0
                                  file:text-sm file:font-semibold
