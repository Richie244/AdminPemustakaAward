...
@section('content')
        ...
        <div class="flex justify-between items-center mb-8 border-b pb-4">
            <div class="flex items-center space-x-3">
                <a href="{{ route('validasi.aksara.index', request()->query()) }}"
                    ...
                    title="Kembali ke Daftar Validasi">
            ...
            <div class="space-y-5">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <p class="font-medium">{{ $peserta->dosen_usulan ?? ($peserta->DOSEN_USULAN ?? 'Tidak tersedia') }}
                        </p>
                    </div>
                    <div>
                        <label class="text-sm text-gray-500 block">Link Media Sosial/Lampiran</label>
                        @if ($peserta->link_upload && $peserta->link_upload !== '#')
                            <a href="{{ $peserta->link_upload }}" target="_blank" rel="noopener noreferrer"
    ...
    --}}
        ...
        @if (strtolower($peserta->STATUS ?? 'pending') === 'ditolak' &&
                !empty(trim((string) ($peserta->ALASAN_PENOLAKAN ?? ''))))
        ...
        @if (strtolower($peserta->STATUS ?? 'pending') !== 'pending' &&
                (isset($peserta->VALIDATOR_ID) || isset($peserta->TGL_VALIDASI)))
        ...
        {{-- Modal Penolakan --}}
        @if (strtolower($peserta->STATUS ?? 'pending') === 'pending')
            <div x-show="showRejectModal" x-cloak x-transition:enter="ease-out duration-300"
                x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
                x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100"
                x-transition:leave-end="opacity-0"
                class="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-50">
                <div @click.outside="showRejectModal = false" x-transition:enter="ease-out duration-300"
                    ...
                    <form method="POST" action="{{ route('validasi.aksara.tolak', ['id' => $peserta->id]) }}">
                        ...
                        <textarea name="alasan"
                            ...
                            placeholder="Tulis alasan penolakan..." rows="4" required></textarea>
