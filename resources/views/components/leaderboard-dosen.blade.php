...
<div class="w-full">
    @if (!empty($top5Dosen))
        ...
        <div class="flex justify-between items-center mb-4">
            <h2 class="text-3xl font-bold font-rubik">Leaderboard Dosen</h2>
        </div>
                ...
                @if ($topUser)
                    <div class="text-center">
                        <div class="relative w-36 h-36 mx-auto">
                            ...
                            <div class="absolute top-0 right-0 bg-[rgba(251,195,77,1)] text-[#880e4f] font-bold text-sm px-3 py-1 rounded-full shadow-md font-rubik">
                                1st
                            </div>
                        </div>
            ...
            <table class="w-full text-gray-700 border-collapse table-fixed">
                <thead class="p-0">
                    <tr class="bg-white border-t border-gray-300">
                        ...
                        <th class="px-2 py-3 font-rubik text-left border-t border-gray-300 w-[20%] text-sm">NIK</th>
                ...
                <tbody>
                    @foreach ($top5Dosen as $index => $user)
                        @php
                        ...
                        <tr class="{{ $index % 2 == 0 ? 'bg-gray-100' : 'bg-white' }}">
                            <td class="px-2 py-3">
                                <span class="inline-flex items-center justify-center w-8 h-8 rounded-full bg-[rgba(251,195,77,1)] text-[#880e4f] font-medium border border-gray-300 text-sm font-rubik">
                                    {{ $index + 1 }}
                                </span>
                            </td>
                            <td class="px-2 py-3">
                            ...
                            <td class="px-2 py-3 font-rubik font-bold text-[#880e4f] truncate text-sm" title="{{ $user['nama'] ?? '-' }}">{{ $user['nama'] ?? '-' }}</td>
                            <td class="px-2 py-3 font-rubik font-bold text-[#880e4f] text-xs">{{ $user['nim'] ?? '-' }}</td>
                            <td class="px-2 py-3 font-rubik font-bold text-[#880e4f] text-xs">{{ $user['status'] ?? '-' }}</td>
                            <td class="px-2 py-3 text-center text-[#880e4f]">
    ...
    @else
        <div class="bg-white p-6 rounded-xl shadow-lg text-center text-gray-500">
            <p class="font-rubik">Data leaderboard dosen tidak tersedia.</p>
        </div>
    @endif
</div>
