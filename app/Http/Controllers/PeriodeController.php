...
class PeriodeController extends Controller
{
    ...
    {
        $labelPoinKomponen = [];
        $komponenBobotIdsUntukForm = [4, 5, 6, 7, 8]; // ID_JENIS_BOBOT yang relevan untuk bagian "Pengaturan Poin/Bobot Komponen"
        foreach ($this->jenisBobotMapping as $key => $idBobot) {
            // Hanya ambil yang termasuk dalam $komponenBobotIdsUntukForm
            ...
            try {
                ...
                if ($allPeriodesCollection->isNotEmpty()) {
                    $latestPeriode = $allPeriodesCollection->sortByDesc(function($periode) {
                        // Pastikan ID yang digunakan untuk sorting ada dan numerik
                    ...
                    if ($latestPeriode) {
                        ...
                        // Panggil helper function untuk mendapatkan detail periode beserta relasinya
                    ...
                    Log::info('[PERIODE_CREATE] Tidak ada data periode tersedia untuk dijadikan referensi setting sebelumnya.');
                }
            } catch (\Exception $e) {
    ...
    private function formatDataForPreviousSettingsForm(array $details): array
    {
        ...
        $pembobotans = $details['pembobotans'] instanceof Collection ? $details['pembobotans'] : new Collection(); 
        
        Log::info('[PREVIOUS_SETTINGS_FORMAT] Data Pembobotan diterima (sebelum diproses):', $pembobotans->toArray());
        ...
        if ($periode) {
            ...
            try {
                $settings['start_date'] = $tglMulaiOri ? Carbon::parse($tglMulaiOri)->format('Y-m-d') : '';
                $settings['end_date'] = $tglSelesaiOri ? Carbon::parse($tglSelesaiOri)->format('Y-m-d') : '';
            } catch (\Exception $e) {
        ...
        $settings['rewards'] = [];
        Log::info('[PREVIOUS_SETTINGS_FORMAT] Data Rewards diterima (sebelum diproses):', $details['rewards'] ?? []);
        
        foreach (($details['rewards'] ?? []) as $reward) {
            $reward = (object) $reward; // Pastikan objek
            ...
            if ($level && in_array((int)$level, [1, 2, 3])) {
                ...
                $skorMinimalEntry = $pembobotans->get($idJenisBobotSkorMinimal); // Ambil dari $pembobotans
                Log::info("[PREVIOUS_SETTINGS_FORMAT] Reward Level {$levelInt}, ID Jenis Bobot Skor: {$idJenisBobotSkorMinimal}, Skor Minimal Entry:", $skorMinimalEntry ? (array)$skorMinimalEntry : 'Tidak Ditemukan');
                ...
                Log::warning('[PREVIOUS_SETTINGS_FORMAT] Reward item tidak memiliki level yang valid atau level tidak di 1,2,3:', (array)$reward); 
            }
        }
        // Pastikan semua level reward ada di array, meskipun kosong
        for ($i=1; $i<=3; $i++) { 
        ...
        foreach ($this->jenisBobotMapping as $formKey => $idBobot) {
            if ($idBobot >= 4) { // Hanya untuk poin komponen (maks kunjungan, dll.)
        ...
        Log::info('[PERIODE_CREATE] Previous settings formatted for form:', $settings);
        return $settings;
    }
